...
export function BOH() {
  const { tickets, fulfillTicket, bohStaff } = useMissionStore();
        ...
        <div className="px-6 py-4">
          <div className="flex gap-6">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2">
              {bohStaff.map((member) => (
                <BOHStaffCard key={member.id} member={member} />
              ))}
            </div>
...
function BOHStaffCard({ member }: { member: StaffMember }) {
      ...
      <div className={`rounded-lg border ${getBorderColor()} bg-white ${member.overtimeMinutes === 0 ? 'shadow-sm' : ''} p-3`}>
        ...
        <div className="mb-2 p-2 bg-[#F8F7F4] rounded border border-[#E8E6DD]">
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <div className="flex items-center gap-1">
              <span className="text-[#1A1A1A] font-medium">${member.hourlyWageUsd}/hr</span>
            </div>
          </div>
        </div>
